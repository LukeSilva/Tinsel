...
echo "#pragma once
#include \"tnl2c.h\" 
$mainfunction" | cat - $1 | sed --posix -e "s/\([a-zA-Z0-9]*\) array/Tinsel_array\<\1\>/g" -e "s/import \(.*\)/#include \\\"\1.h\\\"/g" -e "s/extends/\: public/g" -e "s/self\./this\-\>/g" -e "s/self\./this->/g" -e "s/String /__string__/g" -e "s/\([a-zA-Z]*\)(\([A-Z][a-zA-Z0-9_<>]*\), \([A-Z][a-zA-Z0-9_<>]*\) \(.*\))/\1(\2\&, \3& \4)/g" -e "s/\([a-zA-Z]*\)(\([A-Z][a-zA-Z0-9_<>]*\) \(.*\))/\1(\2\& \3)/g" -e "s/Tinsel_array/tinsel_array/g" -e "s/__string__/String /g" > $2
